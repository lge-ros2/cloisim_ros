...
TEST_F(ContactNodeTest, ReceivesAndPublishesContactData)
{
  ...
  ros_gz_interfaces::msg::Contacts received_msg;
  ...
  ASSERT_EQ(received_msg.contacts.size(), 1u);
  EXPECT_EQ(received_msg.contacts[0].collision1.name, "link1::collision");
  EXPECT_EQ(received_msg.contacts[0].collision2.name, "link2::collision");
}
